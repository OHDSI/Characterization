select t.*, op.observation_id
into #target_cohort
from
@target_database_schema.@target_table t
inner join
(select person_id, observation_period_start_date,
observation_period_end_date,
ROW_NUMBER() OVER(PARTITION BY person_id ORDER BY observation_period_start_date asc) observation_id
from
@cdm_database_schema.observation_period) op
on op.person_id = t.subject_id
and t.cohort_start_date <= op.observation_period_end_date
and t.cohort_end_date >= op.observation_period_start_date
where t.cohort_definition_id in (@target_ids)
;

select o.*, op.observation_id
into #outcome_cohort
from
@outcome_database_schema.@outcome_table o
inner join
(select person_id, observation_period_start_date,
observation_period_end_date,
ROW_NUMBER() OVER(PARTITION BY person_id ORDER BY observation_period_start_date asc) observation_id
...
and o.cohort_end_date >= op.observation_period_start_date
where o.cohort_definition_id in (@outcome_ids)
;
...
from (select *, row_number() over (partition by cohort_definition_id, subject_id order by cohort_start_date) as era_number from #target_cohort)  dc0
  ...
  on dc0.subject_id = dc1.subject_id and dc0.observation_id = dc1.observation_id -- new
  ...
  and dc0.era_number = 1
	...
	on dc1.subject_id = io1.subject_id and dc1.observation_id = io1.observation_id -- new
	...
	on dc1.subject_id = ro0.subject_id and dc1.observation_id = ro0.observation_id -- new
	...
	on dc1.subject_id = de1.subject_id and dc1.observation_id = de1.observation_id -- new
	...
	on de1.subject_id = ro1.subject_id and de1.observation_id = ro1.observation_id -- new
