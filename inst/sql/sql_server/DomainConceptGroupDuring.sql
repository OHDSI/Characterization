...
SELECT covariate_id,
	CAST(CONCAT('@domain_table group (@analysis_name) starting between cohort start and cohort end: ', CASE WHEN concept_name IS NULL THEN 'Unknown concept' ELSE concept_name END {@sub_type == 'inpatient'} ? {, ' (inpatient)'}) AS VARCHAR(512)) AS covariate_name,
